...
for i in range(0,len(variables)):
	...
	for cut1 in np.arange(variables[i]['iRange'], variables[i]['fRange'], variables[i]['step1']):
	   for cut2 in np.arange(variables[i]['uRange'], variables[i]['fRange'], variables[i]['step2']):
		...
		OutRootFileName = 'histo_El_'+str(variables[i]['name'])+'_'+(str(cut1).replace(".","_")).replace("-","_")+'_'+(str(cut2).replace(".","_")).replace("-","_")+'.root'
		...
		print cut1,"-",cut1,"\t",output.strip(),"\t",output_aQGC.strip()
	SigOutFile.close()			
	AllOutFile.write("\n\n")
